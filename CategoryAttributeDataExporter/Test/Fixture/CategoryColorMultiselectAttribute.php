...
/**
 * Same as CategoryColorAttribute but uses a `multiselect` attribute (varchar backend,
 * comma-separated option ids). Creates four categories: blue, yellow, both, and none.
 *
 * Used by CategoryAttributeMultiselectOptionLabelChangeTest.
 */
class CategoryColorMultiselectAttribute implements RevertibleDataFixtureInterface
{
    public const STORE_CODE = 'fixture_second_store';
    public const ATTRIBUTE_CODE = 'category_color_multi';

    public const BLUE_CATEGORY_ID = 9011;
    public const YELLOW_CATEGORY_ID = 9012;
    public const BOTH_CATEGORY_ID = 9013;
    public const NO_ATTR_CATEGORY_ID = 9014;
    ...
    public function apply(array $data = []): ?DataObject
    {
        ...
        [$blueOptionId, $yellowOptionId] = $this->ensureCategoryColorMultiAttribute($secondStoreId);
    ...
    public function revert(DataObject $data): void
    {
        ...
        $ids = [
            self::BLUE_CATEGORY_ID,
            self::YELLOW_CATEGORY_ID,
            self::BOTH_CATEGORY_ID,
            self::NO_ATTR_CATEGORY_ID,
        ];
        foreach ($ids as $id) {
            try {
     ...
     */
    private function ensureCategoryColorMultiAttribute(int $secondStoreId): array
    {
        ...
        if (!$existing->getId()) {
            ...
            $attribute->setData([
                ...
                'is_unique'       => 0,
                'is_required'     => 0,
                'frontend_label'  => ['Category Color Multi'],
                'backend_type'    => 'varchar',
                'source_model'    => \Magento\Eav\Model\Entity\Attribute\Source\Table::class,
                'option'          => [
                    'value' => [
                        'category_color_multi_blue'   => [self::ADMIN_LABEL_BLUE],
                        'category_color_multi_yellow' => [self::ADMIN_LABEL_YELLOW],
                    ],
                    'order' => [
                        'category_color_multi_blue'   => 1,
                        'category_color_multi_yellow' => 2,
                    ],
                ],
            ]);
            $attribute->save();
    ...
    private function resolveOptionIds(): array
    {
        ...
        if ($blueId === null || $yellowId === null) {
            throw new \RuntimeException('Failed to resolve category_color_multi option ids during fixture setup.');
        }
    ...
    private function createCategories(int $blueOptionId, int $yellowOptionId): void
    {
        $categories = [
            [
                ...
                'name'  => 'Category Color Multi Blue',
                'value' => (string)$blueOptionId,
            ],
            [
                'id'    => self::YELLOW_CATEGORY_ID,
                'name'  => 'Category Color Multi Yellow',
                'value' => (string)$yellowOptionId,
            ],
            [
                'id'    => self::BOTH_CATEGORY_ID,
                'name'  => 'Category Color Multi Both',
                'value' => $blueOptionId . ',' . $yellowOptionId,
            ],
            [
                'id'    => self::NO_ATTR_CATEGORY_ID,
                'name'  => 'Category Color Multi None',
                'value' => null,
            ],
        ];
        ...
        foreach ($categories as $data) {
            ...
            $category->setId($data['id'])
                ...
                ->setStoreId(0);
            if ($data['value'] !== null) {
                $category->setData(self::ATTRIBUTE_CODE, $data['value']);
            }
            $category->save();
        }
    }
